...
public class JsonObject<K, V> extends LinkedHashMap<K, V> {
	...
	/**
	 * @return boolean true if this object references another object, false
	 *         otherwise.
	 */
	...
	public Long getReferenceId() {
		...
		Long v = (long) -1;
		if (get("$ref").getClass().equals(String.class))
			v = Long.parseLong((String) get("$ref"));
		else
			v = (Long) get("$ref");
	...
	public boolean isCollection() {
		...
		if (containsKey("$values") && !containsKey("$keys")) {
	...
	public boolean isArray() {
		if (target == null) {
			...
			return containsKey("$values") && !containsKey("$keys");
		}
		return target.getClass().isArray();
	}

	// Return the array that this JSON object wraps. This is used when there is a
	// Collection class (like ArrayList)
	// represented in the JSON. This also occurs if a specified array type is used
	// (not Object[], but Integer[], for
	// example).
	...
	public int getLength() {
		if (isArray()) {
			if (target == null) {
				Object[] items = (Object[]) get("$values");
	...
	void moveCharsToMate() {
		...
		if (items == null) {
			...
			throw new JsonIoException("char[] should only have one String in the [], found " + items.length + ", line "
					+ line + ", col " + col);
		}
	}
	...
	public V put(K key, V value) {
		...
		if (key.equals("$type")) {
			...
			if (value.getClass().equals(String.class))
				...
				id = (Long) value;
			return (V) oldId;
		} else if (("$values".equals(key) && containsKey("$keys")) || ("$keys".equals(key) && containsKey("$values"))) {
			isMap = true;
		}
	...
	public int size() {
		if (containsKey("$values")) {
			Object value = get("$values");
